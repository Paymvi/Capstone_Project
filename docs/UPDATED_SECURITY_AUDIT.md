...
| VULN-004 | Equip items not in inventory | High | Yes | Browser console `fetch()` | Solved | After adding a server-side ownership check to `PUT /equip`, attempts to equip items that are not present in the user’s inventory now return `403 Forbidden`. The avatar equipment is not updated.
| VULN-005 | All marker coordinates revealed on login | High | Yes | Browser console `fetch()` | Solved | The `/me/state` endpoint was updated so it no longer returns all marker coordinates. Marker discovery was moved to a dedicated `/markers/nearby` endpoint that requires the user’s current latitude and longitude and only returns markers within an approved reveal radius. Full marker access is restricted to an admin-only route. Server-side collection distance checks remain enforced to prevent frontend bypasses.
| VULN-006 | Username enumeration via error messages | High | **(No Login Required)** | Browser console `fetch()` | Solved | The login endpoint should not disclose whether the username or password was incorrect. All failed authentication attempts should return the same generic message and status code, such as `401 Invalid credentials`. Detailed reasons should only be logged server-side for administrators.
| VULN-007 | Unlimited brute-force login, no rate limit | High | **(No Login Required)** | Browser console loop | Solved | Added rate limiting to authentication routes using `express-rate-limit`. Login and registration attempts are now limited to 5 attempts per 15-minute window per IP address. Added a general API rate limiter to reduce abuse across the application. Existing account lockout logic also locks accounts after repeated failed login attempts. Verified that repeated login attempts now return `HTTP 429 Too Many Requests` instead of allowing unlimited attempts
| VULN-008 | Register with 1-character password | High | **(No Login Required)** | Browser console `fetch()` | Solved | Implemented server-side input validation using Zod for the registration and login schemas. The registration endpoint now rejects usernames and passwords that are too short or too long, and passwords must include uppercase letters, lowercase letters, and numbers. Invalid registration attempts return `400 Bad Request` before the account is created, preventing weak passwords such as `"a"` from being accepted.
| VULN-009 | JWT readable from `localStorage` | Medium | Yes | DevTools → Application | Solved | Fixed the vulnerability by removing JWT storage from `localStorage` and storing the authentication token in an `HttpOnly cookie` instead. Frontend requests now use credentials: "include", and the backend reads the token from `req.cookies` so JavaScript can no longer access the JWT directly.
| VULN-010 | Admin route has no server-side guard | Medium | Yes | Browser navigation to `/admin` | Solved | The `/admin` frontend route is accessible to regular users, but admin functionality remains protected only if the backend `/admin/markers` endpoint enforces an admin role check. The frontend is_admin conditional should be treated as UX only, not a security boundary.
| VULN-011 | Create arbitrary users unauthenticated | Medium | **(No Login Required)** | Browser console `fetch()` | Solved | The `/users` route was protected by adding `authMiddleware` and `requireAdmin`. This prevents unauthenticated users from creating fake user records because every request must now include a valid authentication token. Even if a user is logged in, the route also verifies that the user has admin privileges before allowing a new user record to be created.
| VULN-012 | Verbose DB errors leak schema details | Medium | **(No Login Required)** | Browser console `fetch()` | Solved | The solution was to secure the `/users` route so malformed requests can no longer create invalid users or expose internal database errors. The route now requires authentication and admin authorization using `authMiddleware` and `requireAdmin`. It also validates the request body before running the database insert, making sure username, email, and name are all provided.
